...
let permute = (p, n) =>
  ...
  |> Sequence.iter(~f=i =>
       let target = Random.int(i);
       Array.swap(p, i, target);
     );

let perlin_generate_perm = () => {
  let p = Array.create(~len=points_count, 0);
  let () =
    Sequence.range(0, points_count)
    |> Sequence.iter(~f=i =>
         let () = Array.unsafe_set(p, i, i);
...
let create = () => {
  let ranfloat =
    Sequence.range(0, points_count)
    |> Sequence.map(~f=_ => Random.float(1.))
    |> Sequence.to_array
...
let noise = (p: Vec3.t, {ranfloat, perm_x, perm_y, perm_z}: t): float => {
  ...
  Array.unsafe_get(
    ranfloat,
    Int.bit_xor(
      Int.bit_xor(Array.unsafe_get(perm_x, i), Array.unsafe_get(perm_y, j)),
