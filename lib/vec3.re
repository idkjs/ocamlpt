...
let length_square = v1 => dot(v1, v1);

let length = v1 => Float.sqrt(length_square(v1));

let negate = v => {x: -. v.x, y: -. v.y, z: -. v.z};
...
let refract = (uv, n, etai_over_etat) => {
  open Float;
  let cos_theta = dot(negate(uv), n);
  let r_out_parallel = etai_over_etat *| (uv +| cos_theta *| n);
  let r_out_perp = - sqrt(1.0 - length_square(r_out_parallel)) *| n;
